...
/// iPadOS のサイドバー（フォルダツリー）。ライブラリ項目とフォルダ階層を表示する（F-ORG-02）。
struct PadSidebarView: View {
    ...
    var body: some View {
        List(selection: selectionBinding) {
            ...
            Section("フォルダ") {
                PadFolderTree(sidebar: sidebar, parentID: nil, depth: 0, onRename: beginRename)
            }
        }
...
/// フォルダ階層を再帰的に描画するビュー（iPadOS）。
private struct PadFolderTree: View {
    ...
    var body: some View {
        ForEach(sidebar.childFolders(of: parentID)) { folder in
            ...
            PadFolderTree(sidebar: sidebar, parentID: folder.id, depth: depth + 1, onRename: onRename)
        }
    }
}
