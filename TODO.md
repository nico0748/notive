...
- 手書きの高度な編集: 投げ縄選択（F-INK-06、ツールピッカー標準）、図形補正（F-INK-08、
  ...
  罫線・方眼・ドット・五線譜・コーネル式）
- PDF 読み込み（F-PDF-01）・ノート内埋め込み表示（F-PDF-02、複数ページ・サムネイル一覧）・
  PDF 本文の全文検索統合（F-PDF-08、取り込み時に PDFKit で抽出）
- バージョン履歴（F-EDIT-08、直近30日）: 自動保存ごとにスナップショットを記録し、
  履歴シートから過去の版へ復元できる。連続する保存は60秒窓でまとめ、保持期間切れの版は
  起動時に自動削除する
- NotesCore のユニットテスト
...
### 第3弾（PDF）の残り
- ビューア内のサムネイル一覧サイドバー（`PDFThumbnailView`）。現状はカードに先頭ページの
  サムネイルを表示し、ビューアは `PDFView` の連続スクロールで複数ページを表示する。
- PDF 注釈: 手書き（F-PDF-04）、テキストボックス配置（F-PDF-03）、図形挿入（F-PDF-05）、
  テキストハイライト・下線・取り消し線（F-PDF-06）。
- 注釈付き PDF のエクスポート（F-PDF-07）、ページ単位の回転・並び替え・削除・追加（F-PDF-09）、
  注釈の表示／非表示切り替え（F-PDF-10）。
- PDF バイト列は現状 `PdfBlock` へインライン保持。大容量ノートでは SwiftData の
  別エンティティ化（遅延読み込み）を検討する（`InkBlock` と同じ申し送り）。
